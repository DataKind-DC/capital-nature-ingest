...
def get_event_ids(num_months):
    ...
    for month_offset in range(num_months):
        ...
        # doubleknot require the word "JSON" to be prepended to the stringified JSON data
        response = requests.post('https://usbg.doubleknot.com/app/calendar/eventdata', params=req_params,
...
def get_event_info(evt_id):
    '''
    Given an event id, go to the event detail page and extract all its information
    :param evt_id: id of event, used in event detail page url
    :return: dict of event information
    '''
    url = f"https://usbg.doubleknot.com/registration/calendardetail.aspx?activitykey={evt_id}&orgkey=4153"
    ...
    if cost_span is not None:
        cost = re.search(r"([0-9\.]+) per non-?member", cost_span.get_text().lower()).group(1)
    ...
    # get the rest of the info, which unfortunately is embedded in a sequence of p elements without identifiers
    # except the content and position
    img_host = "https://5a6a246dfe17a1aac1cd-b99970780ce78ebdd694d83e551ef810.ssl.cf1.rackcdn.com/"
    description, featured_image, venue, evt_name, evt_category = "", "", "", "", ""
    # there is only one element in divNotes with class dk_cd_itemdetail
    right_panel = soup.find("div", {"id": "divNotes"}).find("div", {"class": "dk_cd_itemdetail"})
    ...
    for idx, p in enumerate(right_panel.find_all("p")):
        ...
        if len(images) > 0:
            ...
            # I thought I could get the evt name and category from the th with class dk-title-bar, but it's not
            # formatted consistently. The fist p element is, like this - CATEGORY: TITLE
            ...
            # Add all text that doesn't start with some kind of label and isn't the subtitle to the description
            description += text+"\n"
