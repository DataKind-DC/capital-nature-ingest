...
def get_event_description(event_li):
    ...
    try:
        start_desc_ix = event_li.renderContents().index(b'description"&gt;')
    except ValueError:
        #subsection not found so try another
        start_desc_ix = event_li.renderContents().index(b'description&quot;&gt')
    start_desc_ix += len('description"&gt;')
    desc = rendered_content[start_desc_ix:].strip().decode()
...
def main():
    ...
    event_names = [e.get_text().replace('WAITLIST - ','') for e in event_names]
